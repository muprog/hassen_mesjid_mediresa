...
//   const { payments, isLoading, error, successMessage } = useSelector(
...
//   useEffect(() => {
//     if (payment) {
//       setPaymentType(payment.paymentType?._id ?? '')
//       setAmountDue(payment.amountDue ? String(payment.amountDue) : '')
//       setAmountPaid(payment.amountPaid ? String(payment.amountPaid) : '')
...
//       router.push('/committee/payments')
//     }
//   }, [successMessage, dispatch, router])

//   // when a type is picked and amountDue is empty, prefill from default
//   useEffect(() => {
...
//     }
//   }, [paymentType, paymentTypes, amountDue])

//   if (!payment) {
//     return (
//       <div className='text-center py-12 text-gray-500'>Loading payment...</div>
//     )
//   }
...
//           <button
//             type='button'
//             onClick={() => router.push('/committee/payments')}
...
export default function EditPaymentPage() {
  ...
  // prefill amount due from default when a type is picked
  useEffect(() => {
    if (!paymentType) return
    const t = paymentTypes.find((pt) => pt._id === paymentType)
    if (t && (!amountDue || amountDue === '0')) {
